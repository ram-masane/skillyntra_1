...
export default function WorkspaceView({ role, section, selectedCourse }: { role: string; section: string; selectedCourse: string }) {
  ...
  useEffect(() => {
    ...
    if (["course-health", "recommendations"].includes(section)) {
      if (!selectedCourse) setAlignment(null);
      else fetch(`${API}/api/institute/programs/${encodeURIComponent(selectedCourse)}`).then((response) => response.json()).then(setAlignment).catch(() => setError("Course health could not be loaded."));
    }
